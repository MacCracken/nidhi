...
Rust 1.1.0 crate. The oracle (`rust-old/`, 7180 lines) was retired in 2.0.4 — see below.
...
- **The correctness bar is still "matches what Rust did"** — but `rust-old/` is no longer in the
  working tree (removed 2.0.4, after a full port-coverage audit: 293/328 public items ported, no
  falsifiable symbol-level gap). Cross-check against, in order:
  1. `tests/golden.tcyr` — 43 assertions against values captured from a live oracle build
     (`tests/golden-oracle.txt`): stretch values, WSOLA/OLA interiors, `.round()` ties,
     `detect_loop_points` ordering.
  2. `git show <rev>:rust-old/src/<mod>.rs` — the full Rust source is in git history forever.
     `git log --all -- rust-old/` finds the revisions.
  3. [`docs/port/oracle-build-identity.md`](docs/port/oracle-build-identity.md) — the exact
     dependency versions and rustc the oracle was built with. **The oracle linked naad 1.2.5**,
     a different major version from the naad the port links today; any "did Rust really do X?"
     question must be answered against 1.2.5.
  To re-derive golden vectors, restore the crate from git and build a throwaway crate that
  depends on it **by path** — never modify a restored oracle tree.
